		...
		<div class="d-flex gap-2 flex-wrap flex-column flex-md-row {{{ if config.theme.centerHeaderElements }}}justify-content-center{{{ else }}}justify-content-between{{{ end }}}">
			<div class="d-flex flex-column gap-3">
				...
				<div class="topic-info d-flex gap-2 align-items-center flex-wrap {{{ if config.theme.centerHeaderElements }}}justify-content-center{{{ end }}}">
					<span component="topic/labels" class="d-flex gap-2 {{{ if (!scheduled && !pinned && !locked && !icons.length && (!oldCid || (oldCid == "-1")) && !isQandA) }}}hidden{{{ end }}}">
						...
						<span component="topic/resolved" class="badge badge border qa-status-resolved {{{ if (!isQandA || !resolved) }}}hidden{{{ end }}}">
							<i class="fa fa-check-circle"></i> [[topic:resolved]]
						</span>
						<span component="topic/unresolved" class="badge badge border qa-status-unresolved {{{ if (!isQandA || resolved) }}}hidden{{{ end }}}">
							<i class="fa fa-question-circle"></i> [[topic:unresolved]]
						</span>
